...
proc transExp*(hasErr: var bool, venv: var VEnv, tenv: var TEnv,
    ...
    #     let (,_rhs) = transExp(hasErr, venv, tenv, e.aexp)
    #     if rhs.kind == NilT and lhs.kind != RecordT:
    #         error hasErr, e.apos, "nil can only be assigned to a record type"
    #     if lhs.kind != rhs.kind:
    #         error hasErr, e.apos, "attempting to assign type ", rhs.kind, " to ", lhs.kind
    ...
    #         venv.enter(e.fvar, EnvEntry(kind: VarEntry, ty: Type(kind:IntT, assignable:false)))
    ...
    #         error hasErr, e.arrpos, "array initializer type is ", tyinit.kind, " but array is declared with type ", aty.ty
    #     return (42, aty.ty)
